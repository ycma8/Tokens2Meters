...
def eval_f1_and_geo(dataset, delta: float = 2.0):
...
def _parse_deltas(arg_val: str, fallback: float):
    """Parse comma-separated thresholds; if empty/invalid, return [fallback]."""
    if arg_val is None:
        return [fallback]
    try:
...
def eval_multi_deltas(dataset, deltas):
    """Run eval_f1_and_geo for multiple deltas and average the metrics."""
    per_delta = {}
    for d in deltas:
        per_delta[d] = eval_f1_and_geo(dataset, delta=d)

    # Averages across deltas
    avg_micro = {
    ...
    for c in sorted(classes):
        ...
        for d in deltas:
            if c in per_delta[d]["per_class"]:
                ...
                if tp0 is None:
                    tp0 = per_delta[d]["per_class"][c]["TP"]
                    fp0 = per_delta[d]["per_class"][c]["FP"]
                    fn0 = per_delta[d]["per_class"][c]["FN"]
        avg_per_class[c] = {
            "Precision": _nanmean_list(Ps),
            "Recall":    _nanmean_list(Rs),
            "F1":        _nanmean_list(Fs),
            "GT": int((tp0 or 0) + (fn0 or 0))
        }
        # Geometry averages
        As, Ss, Os = [], [], []
        for d in deltas:
...
def main():
    ...
    ap.add_argument("--delta",  type=float, default=2.0, help="single center-distance threshold (m) for F1/geo")
    ap.add_argument("--deltas", type=str,   default=None, help="comma-separated thresholds, e.g., '1,2,3,5'")
    args = ap.parse_args()

    dataset = load_dataset(args.file)

    # If --deltas is given, use multi-threshold path; else keep single-threshold behavior
    ...
    deltas = _parse_deltas(args.deltas, fallback=args.delta)
    per_delta, averaged = eval_multi_deltas(dataset, deltas)

    # Print per-delta summaries
    for d in deltas:
        out = per_delta[d]
    ...
    # Print averaged results across deltas
